...
public class WnckCoffeeMakerService implements CoffeeMakerService, AdminCoffeeMakerService {
   ...
   */
  @Override
  public List<CoffeeMakerConfiguration> getCoffeeMakers() {
    logger.info("getCoffeeMakers: no params");
    List<CoffeeMakerConfiguration> results = new ArrayList<>();
    makers.values().forEach((maker) -> results.add(maker.getConfiguration()));
   ...
   */
  @Override
  public CoffeeMakerConfiguration getCoffeeMaker(final int id) throws IllegalParameterException {
    logger.info("getCoffeeMaker: id = {}", id);
    CoffeeMaker maker = getMakerById(id);
    return maker.getConfiguration();
  }
  ...
  @Override
  public State getState(final int id) throws IllegalParameterException, IllegalStateException {
    logger.info("getState: id = {}", id);
    CoffeeMaker maker = getMakerById(id);
    return maker.getState();
  }

  @Override
  public synchronized void addCoffeeMaker(final CoffeeMakerConfiguration configuration) throws
          IllegalParameterException {
    logger.info("addCoffeeMaker: descriptor = {}", configuration);
    Objects.requireNonNull(configuration);
    CheckingHelper.assertThatIsReadableString(configuration.location(), "location");
    CheckingHelper.assertThatIsReadableString(configuration.model(), "model");

    CoffeeMakerConfiguration makerConfiguration = configuration;

    if (configuration.id() == NOT_YET_ASSIGNED) {
      // find the maximum of all coffee maker ids
      int maxId = makers.keySet().stream().reduce(Integer::max).orElse(0);
      int newId = maxId + 1;
      makerConfiguration = configuration.withId(newId);
    }

    CoffeeMaker maker = new WnckCoffeeMaker(makerConfiguration);

    // just to make sure ...
    if (!makers.containsKey(maker.getId())) {
      makers.put(maker.getId(), maker); // success
    } else {
      logger.error("newly calculated unique maker id {} is already in the map", maker.getId());
      throw new IllegalStateException("new maker id already exists: " + maker.getId());
    }
  ...
  @Override
  public ServiceState getStateForSave() {
    ServiceState state = new ServiceState();
    for (CoffeeMaker maker : makers.values()) {
      state.addConfiguration(maker.getConfiguration());
    }
    return state;
  }

  @Override
  public void loadState(final ServiceState state)
